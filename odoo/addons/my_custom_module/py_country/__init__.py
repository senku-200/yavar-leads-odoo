...
class ExistingCountries(country.db.Database):
    """Provides access to an ISO 3166 database (Countries)."""

    data_class = country.db.Country
    root_key = "3166-1"
    ...
    def search_fuzzy(self, query: str) -> List[Type["ExistingCountries"]]:
        ...
        def add_result(country: "country.db.Country", points: int) -> None:
        ...
        match_subdivions = country.Subdivisions.match(
        ...
        partial_match_subdivisions = country.Subdivisions.partial_match(
            self=subdivisions, query=query
        )
...
class Scripts(country.db.Database):
...
class Currencies(country.db.Database):
...
class Languages(country.db.Database):
...
class LanguageFamilies(country.db.Database):
...
class SubdivisionHierarchy(country.db.Data):
...
class Subdivisions(country.db.Database):
    ...
    def search_fuzzy(self, query: str) -> List[Type["Subdivisions"]]:
        ...
        def add_result(
            subdivision: "country.db.Subdivision", points: int
        ) -> None:
